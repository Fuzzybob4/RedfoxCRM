...
console.log("🔍 Starting build monitoring for Next.js 15+...\n")
...
const checks = [
    ...
    check: () => {
      try {
        ...
        if (fs.existsSync(configPath)) {
          ...
          console.log("   - Experimental features:", Object.keys(config.experimental || {}))
        } else {
    ...
    check: () => {
      try {
        ...
        if (nextVersion && nextVersion.includes("15")) {
          console.log("   - ✅ Using Next.js 15+ (latest)")
        }
  ...
  {
    name: "Radix UI components check",
    check: () => {
      try {
        const pkg = JSON.parse(fs.readFileSync("package.json", "utf8"))
        ...
        console.log("✅ Radix UI components found:")
        radixComponents.forEach((comp) => {
          console.log(`   - ${comp}: ${deps[comp]}`)
        })

        return radixComponents.length > 0
      } catch (error) {
        console.log("❌ Radix UI check failed:", error.message)
        return false
      }
    },
  },
...
console.log("\n" + "=".repeat(50))
if (allPassed) {
  console.log("🎉 All checks passed! Build should succeed with Next.js 15+.")
  console.log("\n📋 Next steps:")
...
if (process.argv.includes("--continue-build")) {
  if (!criticalFailed) {
    console.log("\n🚀 Starting build process with Next.js 15+...")
    try {
